...
@Service
public class SimPDevService extends FrameBaseService<TbSimPDev> {
    ...
    public void deleteAll(List<String> idList){
        List<TbSimPPort> tbSimPPorts=new ArrayList<TbSimPPort>();
        for(int i=0;i<idList.size();i++){
            tbSimPPorts=simPPortDao.querySimPPort(idList.get(i));//先查找所有的卡槽
            for (TbSimPPort simpport:tbSimPPorts) {
                if (!simpport.getIdxIccid().isEmpty()){
                    simCarDao.deleteSimCard(simpport.getIdxIccid());//删除卡
                }
                simPPortDao.deleteSimPPort(simpport.getKeyID());//删除卡槽
            }
            simPDevDao.deleteSimPDev(idList.get(i));//删除卡池
        }
    }
    ...
    public List<SimPDevOfPortInfoVO> queryAllSimPDevPortInfo() {
        ...
        for (SimPDevOfPortInfoVO devInfo : devPortInfo) {
            ...
            // 16 * 8 ,格式 端口展示
            int xSize = 16, ySize = portInfoMap.size()/16;
    ...
    public List<String[]> getSimPDevSelData() {
        ...
        for (TbSimPDev tbSimPDev : queryAll()) {
            selDatas.add(new String[]{tbSimPDev.getKeySimPDevID(), tbSimPDev.getDevName()});
        }
        return selDatas;
    }
    public Page<TbSimPDev> query(int pageNumber, int pageSize, Map<String, Object> queryParam) {
        Criteria criteria = Criteria.create(getEntityClass());
        TbAgent agent = UserUtils.getUserProfile().getTbAgent();
        if(agent !=null){
        	Criteria cspg= Criteria.create(TbSimPDevGrp.class);
        	cspg.where("idxAgentID", " LIKE ",  new Object[]{agent.getIdxAgentId()+"%"});
        	List<TbSimPDevGrp> list = jdbcDao.queryList(cspg);
        	if(list.size()>0){
	        	Object[] obj = new Object[list.size()];
	        	for (int i = 0; i < list.size(); i++) {
	        		TbSimPDevGrp spg = list.get(i);
					obj[i] = spg.getKeyID();
				}
	        	criteria.where("idxSimPDevGrpID", " IN ", obj);
        	}else{
        		queryParam.put("EQ-|-idxSimPDevGrpID", "ERROR");
        	}
        }
    ...
    public List<Object[]> getsimPDevGrpData(){
    	List<Object[]> selDatas = new ArrayList<Object[]>();
    	List<TbSimPDevGrp> tbSimPDevGrp = simPDevDao.querySimpDevGroupList();
        for (TbSimPDevGrp tbSimPDev : tbSimPDevGrp) {
            selDatas.add(new Object[]{tbSimPDev.getKeyID(), tbSimPDev.getGroupName()});
        }
        return selDatas;
    }
